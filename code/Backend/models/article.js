...
module.exports = class Article {
	constructor(id, name, description, seller) {
		this.id = id;
		this.name = name;
	 ...
	 */
	async save() {
		const { rows } = await db.query('INSERT INTO articles (name, description, seller_id) VALUES (?, ?, ?)', [this.name, this.description, this.seller]);
		this.id = rows[0].id;
		return this;
	}
