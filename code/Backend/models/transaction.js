...
export default class Transaction {
	 ...
	 */
	async save() {
		let seller = await User.findByUsername(this.seller.username);
		let buyer = await User.findByUsername(this.buyer.username);
		let article = await Article.findById(this.article.id);

		if (!seller || !buyer || !article) return null;
		...
		await db.query('insert into transaction(id, seller_id, buyer_id, article_id, created) values(?, ?, ?, ?, ?)', [this.id, seller.id, buyer.id, article.id, this.created]);
		return this;
	}
}
